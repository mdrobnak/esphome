...
void AirConditioner::ParseResponse(uint8_t cmdSent) {
  ...
  if ((RXData[RX_BYTE_PREAMBLE] == PREAMBLE) &&
      ...
      (RXData[RX_BYTE_TO_CLIENT] == TO_CLIENT) &&
    ...
    switch (RXData[RX_BYTE_COMMAND_TYPE]) {
      case CLIENT_COMMAND_QUERY: {
        ...
        set_sensor(this->timer_stop_sensor_,
                   CalculateGetTime(RXData[RX_C0_BYTE_TIMER_STOP]));
        set_sensor(this->error_flags_sensor_,
                   (RXData[RX_C0_BYTE_ERROR_FLAGS1] << 0) |
                       (RXData[RX_C0_BYTE_ERROR_FLAGS2] << 8));
        set_sensor(this->protect_flags_sensor_,
                   (RXData[RX_C0_BYTE_PROTECT_FLAGS1] << 0) |
                       (RXData[RX_C0_BYTE_PROTECT_FLAGS2] << 8));
        break;
      }
      case 0xC4:
