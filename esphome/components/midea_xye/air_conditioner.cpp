...
static void set_sensor(Sensor *sensor, float value) {
  if (sensor != nullptr &&
      (!sensor->has_state() || sensor->get_raw_state() != value))
...
void AirConditioner::control(const ClimateCall &call) {
  ...
  if (call.get_preset().has_value()) this->preset = call.get_preset().value();
  this->publish_state();
...
void AirConditioner::setup() {
  ...
  uint8_t data;
  while (this->uart_->available()) this->uart_->read_byte(&data);
}
...
void AirConditioner::update() {
  if (0 == UpdateNextCycle) {
    ...
    // set mode flags
    TXData[12] = ((this->preset == ClimatePreset::CLIMATE_PRESET_BOOST) *
                  MODE_FLAG_AUX_HEAT) |
                 ((this->preset == ClimatePreset::CLIMATE_PRESET_SLEEP) *
                  MODE_FLAG_ECO) |
...
void AirConditioner::ParseResponse() {
  ...
  if ((PREAMBLE == RXData[RX_BYTE_PREAMBLE]) &&
    ...
    update_property(this->current_temperature,
                    (float)CalculateTemp(RXData[RX_BYTE_T1_TEMP]),
                    need_publish);
    ...
    {
      update_property(this->target_temperature, (float)RXData[RX_BYTE_SET_TEMP],
                      need_publish);
      ...
      if ((this->swing_mode != ClimateSwingMode::CLIMATE_SWING_OFF) !=
        ...
        need_publish = true;
      this->swing_mode = (RXData[RX_BYTE_MODE_FLAGS] & MODE_FLAG_SWING)
                             ? ClimateSwingMode::CLIMATE_SWING_VERTICAL
                             : ClimateSwingMode::CLIMATE_SWING_OFF;
      if (this->preset != preset) need_publish = true;
      this->preset = preset;
    }

    if (need_publish) this->publish_state();

    set_sensor(this->outdoor_sensor_, CalculateTemp(RXData[RX_BYTE_T3_TEMP]));
    set_sensor(this->temperature_2a_sensor_,
               CalculateTemp(RXData[RX_BYTE_T2A_TEMP]));
    set_sensor(this->temperature_2b_sensor_,
               CalculateTemp(RXData[RX_BYTE_T2B_TEMP]));
    set_sensor(this->temperature_3_sensor_,
               CalculateTemp(RXData[RX_BYTE_T3_TEMP]));
    set_sensor(this->current_sensor_, RXData[RX_BYTE_CURRENT]);
    set_sensor(this->timer_start_sensor_,
               CalculateGetTime(RXData[RX_BYTE_TIMER_START]));
    set_sensor(this->timer_stop_sensor_,
               CalculateGetTime(RXData[RX_BYTE_TIMER_STOP]));
    set_sensor(this->error_flags_sensor_,
               (RXData[RX_BYTE_ERROR_FLAGS1] << 0) |
                   (RXData[RX_BYTE_ERROR_FLAGS2] << 8));
    set_sensor(this->protect_flags_sensor_,
               (RXData[RX_BYTE_PROTECT_FLAGS1] << 0) |
                   (RXData[RX_BYTE_PROTECT_FLAGS2] << 8));
...
ClimateTraits AirConditioner::traits() {
  ...
  traits.add_supported_fan_mode(
      ClimateFanMode::CLIMATE_FAN_OFF);  // Can't set it but will be reported
...
void AirConditioner::dump_config() {
  ...
  ESP_LOGCONFIG(Constants::TAG, "  [x] Period: %dms",
                this->get_update_interval());
  ESP_LOGCONFIG(Constants::TAG, "  [x] Response timeout: %dms",
